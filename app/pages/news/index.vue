  ...
  <div class="pt-24 pb-20 bg-slate-50 min-h-screen">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div v-else-if="newsItems && newsItems.length > 0" class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        <article v-for="(item, index) in newsItems" :key="item.id" class="news-card bg-white rounded-2xl shadow-sm hover:shadow-xl border border-slate-100 transition-all duration-300 transform hover:-translate-y-1 flex flex-col h-full overflow-hidden">
          <div class="relative h-56 overflow-hidden flex-shrink-0">
            <img :src="item.image || '/images/team.png'" :alt="item.title_uz" class="w-full h-full object-cover transition-transform duration-700 hover:scale-110" />
