        ...
        <div v-for="dept in departments" :key="dept.id" class="bg-white rounded-2xl p-6 md:p-8 shadow-sm hover:shadow-md border border-slate-100 transition-shadow">
          <div class="flex flex-col md:flex-row gap-6 items-start">
            <div class="w-12 h-12 rounded-full bg-primary-50 flex items-center justify-center flex-shrink-0">
              <span class="text-xl font-bold text-primary-600">{{ dept.name_uz?.charAt(0) }}</span>
            </div>
            <div>
              <h3 class="text-xl font-bold font-heading text-dark mb-3">{{ dept.name_uz }}</h3>
              <p v-if="dept.description_uz" class="text-slate-600 mb-4 whitespace-pre-line text-sm leading-relaxed">{{ dept.description_uz }}</p>
              
              <div v-if="dept.head_name_uz" class="inline-flex items-center gap-3 bg-slate-50 px-4 py-2 rounded-lg border border-slate-100">
                <svg class="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                <div class="text-sm">
                  <span class="text-slate-500 block text-xs">Bo'lim boshlig'i:</span>
                  <span class="font-semibold text-slate-800">{{ dept.head_name_uz }}</span>
                </div>
              </div>
            </div>
          </div>
