      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
        ...
        <div class="lg:col-span-1">
          ...
          <p class="text-sm text-slate-400 leading-relaxed mb-5">
            {{ generalInfo?.name_uz || "O'zbekiston Respublikasi sog'liqni saqlash vazirligi — Andijon viloyati hokimligi sog'liqni saqlash boshqarmasi rasmiy veb-sayti." }}
          </p>
          <!-- Social Links -->
          <div class="flex items-center gap-3">
            <a v-if="generalInfo?.telegram" :href="generalInfo.telegram" target="_blank" class="w-9 h-9 rounded-lg bg-white/5 hover:bg-primary-500 flex items-center justify-center transition-all duration-300 hover:-translate-y-0.5" aria-label="Telegram">
            ...
            <a v-if="generalInfo?.facebook" :href="generalInfo.facebook" target="_blank" class="w-9 h-9 rounded-lg bg-white/5 hover:bg-blue-600 flex items-center justify-center transition-all duration-300 hover:-translate-y-0.5" aria-label="Facebook">
              <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
            </a>
            <a v-if="generalInfo?.instagram" :href="generalInfo.instagram" target="_blank" class="w-9 h-9 rounded-lg bg-white/5 hover:bg-pink-600 flex items-center justify-center transition-all duration-300 hover:-translate-y-0.5" aria-label="Instagram">
            ...
            <a v-if="generalInfo?.youtube" :href="generalInfo.youtube" target="_blank" class="w-9 h-9 rounded-lg bg-white/5 hover:bg-red-600 flex items-center justify-center transition-all duration-300 hover:-translate-y-0.5" aria-label="YouTube">
        ...
        <div>
          ...
          <div class="space-y-4">
            <div class="flex items-start gap-3">
              ...
              <p class="text-sm text-slate-400 leading-relaxed">{{ generalInfo?.address || "710000, Andijon shahar, Amir Umarxon ko'chasi, 19-uy" }}</p>
            </div>
            <div class="flex items-center gap-3">
              ...
              <a :href="`tel:${generalInfo?.phone?.replace(/\D/g,'')}`" class="text-sm text-slate-400 hover:text-white transition-colors">{{ generalInfo?.phone || '(374) 228-42-84' }}</a>
            </div>
            <div class="flex items-center gap-3">
              ...
              <a :href="`mailto:${generalInfo?.email || 'andijon.vssb@ssv.uz'}`" class="text-sm text-slate-400 hover:text-white transition-colors">{{ generalInfo?.email || 'andijon.vssb@ssv.uz' }}</a>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
        <div class="flex flex-col sm:flex-row items-center justify-between gap-3">
          <p class="text-xs text-slate-500">
            © {{ currentYear }} Andijon viloyati sog'liqni saqlash boshqarmasi. Barcha huquqlar himoyalangan.
          </p>
          <div class="flex items-center gap-1 text-xs text-slate-600">
            <span>Ishlab chiquvchi:</span>
            <a href="https://inforte.uz" target="_blank" rel="noopener" class="text-secondary-500 hover:text-secondary-400 font-medium transition-colors">
              INFORTE
            </a>
          </div>
        </div>
...
const govLinks = [
  ...
  { url: 'https://data.gov.uz', label: "Ochiq ma'lumotlar" },
]
</script>
